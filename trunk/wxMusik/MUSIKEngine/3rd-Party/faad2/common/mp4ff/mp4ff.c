...
int32_t mp4ff_num_samples(const mp4ff_t *f, const int32_t track)
{
	...
	} while (++i<f->track[track]->stts_entry_count);
    return total;
}
...
int32_t mp4ff_find_sample(const mp4ff_t *f, const int32_t track, const int64_t offset)
{
	int32_t i=0, co = 0;
	int64_t offset_total = 0;
	mp4ff_track_t * p_track = f->track[track];

	int64_t offset1=offset+mp4ff_get_sample_offset(f,track,0);
	...
	do {
		...
		if (offset1 < offset_total + offset_delta)
		{
			int64_t offset_fromstts = offset1 - offset_total;
			return co + (int32_t)(offset_fromstts / sample_delta);
		}
		else
			offset_total += offset_delta;
		co += sample_count;
	} while (++i<p_track->stts_entry_count);
	return (int32_t)(-1);
}
...
int32_t mp4ff_read_sample_v2(mp4ff_t *f, const int track, const int sample,unsigned char *buffer)
{
	...
	if (mp4ff_set_position(f, mp4ff_sample_to_offset(f, track, sample))<0) return 0;
