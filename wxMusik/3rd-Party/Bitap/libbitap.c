...
#define ALPHABET (1 << (sizeof (unsigned char) * 8))
...
static int ParseRegex (bitapType *b, const unsigned char *regex, const unsigned char *end)
{
  ...
  const unsigned char *start = regex;
  while ((!end || regex < end) && *regex != '\0' && *regex != ')') {
    ...
    if (*regex == '|') {
      ...
      b->l++; /* Create an unmatchable unsigned char to stop shift from LHS to RHS */
      continue;
    }
...
{ /* in is the 'haystack'. 'b' is the needle. e is the maximum of errors. */
  ...
  for (;; in++) { /* Invarient : r[i].bit[j] implies r[i + words].bit[j] */
    ...
    for (i = e; i >= 0; i--) {
      for (carry = 0, j = 0; j < words; j++) {
	tmp = r[i * words + j];
	r[i * words + j] = (tmp << 1) + carry;
	
	if (i < e) r[(i + 1) * words + j] = (r[(i + 1) * words + j] &
	  b->s[*in + ALPHABET * j]) | ((tmp | r[i * words + j]) & b->x[j]);
	/* tmp <=> deletions. r[i * words + j] <=> substitutions */
  ...
  if (bReturn && best) {
	...
	for (carry = 0, j = words - 1; j >= 0; j--) {
	  tmp = r[i * words + j];
	  r[i * words + j] = ((unsigned) (tmp & b->s[**bReturn + ALPHABET *j])
			      >> 1) | carry;
	  if (i < e) r[(i + 1) * words + j] |= r[i * words + j] | tmp;
	  carry = (tmp & b->s[**bReturn + ALPHABET * j]) << (BITS - 1);
	}
